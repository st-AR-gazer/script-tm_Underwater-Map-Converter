...
if (args.Length < 3)
{
    Console.Error.WriteLine("Usage: dotnet run CloneCustomVistaBlock.cs -- <donorBlockPath> <outputBlockPath> <archetypeBlockInfoId> [collectionId]");
    return;
}
...
item.Description = $"Custom block wrapper for {archetypeBlockInfoId}";
