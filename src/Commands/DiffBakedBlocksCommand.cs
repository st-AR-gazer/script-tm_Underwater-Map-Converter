...
internal static class DiffBakedBlocksCommand
{
    public static int Run(string[] args)
    {
        if (args.Length < 2)
        {
            return CliHelp.Write("Usage: diff-baked-blocks <referenceMapPath> <actualMapPath> [--filter TEXT] [--limit N] [--write-json PATH]");
        }
        ...
        var referenceBlocks = FilterBlocks(referenceMap.BakedBlocks ?? [], filter);
        var actualBlocks = FilterBlocks(actualMap.BakedBlocks ?? [], filter);
        ...
        var referenceChunk = referenceMap.Chunks.Get<CGameCtnChallenge.Chunk03043048>();
        var actualChunk = actualMap.Chunks.Get<CGameCtnChallenge.Chunk03043048>();
        ...
        var report = new
        {
            ...
            actualMapPath,
            filter,
            referenceBakedBlockCount = referenceMap.BakedBlocks?.Count ?? 0,
            actualBakedBlockCount = actualMap.BakedBlocks?.Count ?? 0,
            ...
            referenceChunk03043048 = referenceChunk is null
                ? null
                : new
                {
                    referenceChunk.Version,
                    referenceChunk.BlocksVersion,
                    referenceChunk.U01,
                    bakedClipsAdditionalDataCount = referenceMap.BakedClipsAdditionalData?.Count ?? 0,
                },
            actualChunk03043048 = actualChunk is null
                ? null
                : new
                {
                    actualChunk.Version,
                    actualChunk.BlocksVersion,
                    actualChunk.U01,
                    bakedClipsAdditionalDataCount = actualMap.BakedClipsAdditionalData?.Count ?? 0,
                },
            comparedPairCount,
            ...
            samples,
            note = "Keys are grouped by (typeId|coord|direction|variant|subVariant). For large diffs, use --filter to narrow to a single block family (e.g. DecoWallWaterVFC).",
        };
